...
Reopen (§5.1) is recorded as an open item. `workflow-state.md` §2.4 implements it as a new workflow record linked by `reopened_from` rather than a transition out of a terminal state, because re-enterable terminal states would let a replayed completion event resurrect finished work, which GS-FR-020 forbids. If the intended behavior is a true reopen transition, that requires a new decision and a change to the GS-FR-013 state list.
